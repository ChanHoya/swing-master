...
# 재표본화 주기와 이동평균 창.
#
# 창은 0.10초(30Hz 에서 3샘플)다. 7샘플(0.23초)이던 것을 줄였다 — 다운스윙
# 전체가 0.25초 남짓이라 그만한 창으로 평균을 내면 원호가 통째로 뭉개진다.
# 영상 3개 실측에서 다운스윙 구간 평균 이탈이 17.3px → 13.7px 로 줄었고,
# 대신 떨림(2차 차분)이 1.84px → 2.70px 로 늘었다. 그 거래를 택했다.
RESAMPLE_HZ = 30.0
SMOOTH_WINDOW = 3
