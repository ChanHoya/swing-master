...
- **업로드에 사용자가 붙지 않는다.** `POST /upload`이 인증을 받지 않아
  `Upload.user_id`·`Analysis.user_id`가 항상 `NULL`이다. 그래서 `GET /history`는
  로그인해도 빈 배열을 돌려준다. 기기 간 흐름(모바일 촬영 → PC 상세 분석)의
  선행 조건이며, 설계 문서 §9.2에 상세가 있다.
