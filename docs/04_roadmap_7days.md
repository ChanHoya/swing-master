...
- [x] 궤적 on/off 토글
- [x] 궤적 오검출 보정 — 손목 랜드마크가 다운스윙·임팩트에서 팔꿈치로
      미끄러지는 문제(정상 팔뚝 50px → 10~17px)와, 가시성만 보고 좌/우/중점을
      오가며 점이 튀던 문제(5회 전환, 최대 87px)를 함께 잡았다.
      팔뚝 길이 타당성 검사 + 그립 간격 검사 + 30Hz 재표본화 + 이동평균.
      영상 6개의 "어깨폭 대비 최대 점프"가 0.90~2.15 → 0.49~0.69 로 들어왔다
      (1.0 을 넘으면 사람이 낼 수 없는 속도이므로 오검출이다).
      어깨폭 기준은 중앙값이 아니라 90퍼센타일을 쓴다 — 후면 촬영에서는
      어깨가 접혀 보여 중앙값이 실제의 1/4 까지 작아진다.
